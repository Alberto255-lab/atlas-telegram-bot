...
def handle_command(message):
    ...
    if not is_private:
        ...
        if cmd == "/setsubaffitti":
            if not is_global_admin(user_id):
                return send_message(chat_id, "Non autorizzato.")
            supabase.table("telegram_groups").upsert({
                "chat_id": chat_id, "group_type": "subaffitti", "title": message["chat"].get("title")
            }).execute()
            return send_message(chat_id, "✅ Questa chat è ora impostata come Chat Subaffitti.")
...
def finalize_application(chat_id, from_user, data):
    ...
    notify_employee_group(
        f"📝 <b>Nuova candidatura</b>\nDa: @{username}\nSettore: {data.get('sector')}\nMotivazione: {data.get('motivation')}"
    )
...
def handle_callback(callback):
    ...
    if data_str == "cart:checkout":
        ...
        notify_employee_group(f"🛒 <b>Nuovo ordine (Telegram)</b>\nDa: @{username}\nProdotti: {items_list}\nTotale: €{total:.2f}")
        return
...
def notify_subaffitti_group(text, extra=None):
    res = supabase.table("telegram_groups").select("chat_id").eq("group_type", "subaffitti").execute()
    if not res.data:
        return
    return send_message(res.data[0]["chat_id"], text, extra)
...
# ============================================================
# WEBHOOK DAL DATABASE (ordini/prenotazioni fatti dal sito)
# ============================================================
def handle_database_webhook(body):
    ...
    if table == "orders":
        product = supabase.table("products").select("name, price").eq("id", record["product_id"]).execute().data
        ...
        notify_employee_group(
            f"🛒 <b>Nuovo ordine dal sito</b>\nCliente: {who}\nProdotto: {product.get('name')}\nQuantità: {record.get('quantity', 1)}\nTotale: €{total:.2f}",
    ...
    if table == "applications":
        ...
        notify_employee_group(f"📝 <b>Nuova candidatura dal sito</b>\nCliente: {who}\nSettore: {record.get('sector')}\nMotivazione: {record.get('motivation')}")
